...
!(function (t, e) {
  ...
  const millisecond_per_second = 1e3 //1000 1000ms 等于1 毫秒

  const millisecond_per_minute = millisecond_per_second * minute //60000 1000ms * 60 等于1分钟

  const millisecond_per_hour = millisecond_per_minute * 60 //3600000 1000ms * 60 * 60 等于1小时

  const millisecond_per_day = millisecond_per_hour * 24 //86400000 1000ms * 60 * 60 * 24 等于1天

  const millisecond_per_week = millisecond_per_day * 7 //604800000 1000ms * 60 * 60 * 24 * 7 等于1周

  const millisecond_per_month = millisecond_per_day * 30 //2628000000 1000ms * 60 * 60 * 24 * 30 等于1月

  const millisecond_per_year = millisecond_per_day * 365 //31536000000 1000ms * 60 * 60 * 24 * 365 等于1年
  ...
  const INVALID_DATE_STRING = 'Invalid Date' //不合法的日期字符串

  //日期解析格式化正则
  const REGEX_PARSE = /^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/ //日期解析正则
  const REGEX_FORMAT = /\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS/g //日期格式化正则
  ...
  class Presentjs {
    ...
    add(number, units) {
      ...
      const step =
        {
          [Min]: millisecond_per_minute,
          [H]: millisecond_per_hour,
          [S]: millisecond_per_second,
        }[unit] || 1 //ms
    ...
    diff(input, units, float) {
      // 获取时间差
      const unit = utils.prettyUnit(units)
      const that = presentjs(input)
      const zoneDelta = (that.utcOffset() - this.utcOffset()) * millisecond_per_minute
      ...
      retsult =
        {
          ...
          [M]: result,
          [Q]: result / 3,
          [W]: (diff - zoneDelta) / millisecond_per_week,
          [D]: (diff - zoneDelta) / millisecond_per_day,
          [H]: (diff - zoneDelta) / millisecond_per_hour,
          [MIN]: (diff - zoneDelta) / millisecond_per_minute,
          [S]: (diff - zoneDelta) / millisecond_per_second,
    ...
    clone() {
      return utils.wrapper(new Date(this.$d), this)
    }
    toDate() {
  ...
  const proto = Presentjs.prototype

  presentjs.prototype = proto
  ;[
    ['$ms', MS],
    ['$s', S],
    ['$m', Min],
    ...
    console.log(g)
    // console.log(proto[g[1]])
    proto[g[1]] = function (input) {
      return this.$g(input, g[0], g[1])
    }
  })

  // presentjs.extend = (plugin, option) => {
  //   if (!plugin.$i) {
  //     plugin(option, Presentjs, presentjs)
  //     plugin.$i = true
  //   }
  //   return presentjs
  // }

  presentjs.locale = parseLocale

  presentjs.isPresentjs = isPresentjs

  presentjs.unix = (timestamp) => presentjs(timestamp * millisecond_per_second)
